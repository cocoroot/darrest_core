...
class CreateGoodLogic < LogicBase
  ...
  def authorize(params)
    ...
    @errors.add(:creation, 'Creation does not exist.') unless Creation.exists?(id: params[:good][:creation_id])
    @errors.add(:site_user, 'SiteUser does not exist.') unless SiteUser.exists?(id: params[:good][:site_user_id])
  ...
  def validate(params)
    ...
    @errors.add(:good, 'Good is already created.') if Good.exists?(creation_id: creation_id, site_user_id: site_user_id)
